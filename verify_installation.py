...
def verify_installation():
    ...
    print(
        f"{check_mark(results['python'])} Python {py_version.major}.{py_version.minor}.{py_version.micro}"
    )
    ...
    if results["env_file"]:
        ...
        has_github_token = (
            "GITHUB_TOKEN=" in env_content and "ghp_your" not in env_content
        )
        has_llm_key = (
    ...
    try:
        result = subprocess.run(
            ["autonomous-agent", "--help"], capture_output=True, text=True, timeout=5
        )
    ...
    all_critical_passed = (
        ...
        and results.get("import", False)
        and results.get("core_modules", False)
        and results.get("directories", False)
    )

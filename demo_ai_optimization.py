...
def demo_ml_priority_scorer():
    ...
    test_issues = [
        MockIssue(
            "Critical security vulnerability", 5, 20, ["security", "critical", "bug"]
        ),
...
def demo_anomaly_detector():
    ...
    class MockCommit:
        def __init__(self, msg, additions, deletions, num_files):
            ...
            self.commit = type(
                "C",
                (),
                {"message": msg, "author": type("A", (), {"name": "Developer"})},
            )
            self.stats = type(
                "S",
                (),
            ...
            self.files = [
                type("F", (), {"filename": f"file{i}.py"}) for i in range(num_files)
            ]

    # Establish baseline with normal commits
    baseline_commits = [MockCommit("Fix bug", 15, 8, 2) for _ in range(10)]
    ...
    for name, commit in test_commits:
        ...
        print(
            f"  Lines: +{int(result['features']['lines_added'])} "
...
def demo_commit_summarizer():
    ...
    class MockCommit:
        def __init__(self, msg, author, additions, deletions):
            self.sha = "abc123"
            self.commit = type(
                "C", (), {"message": msg, "author": type("A", (), {"name": author})}
            )
    ...
    commits = [
        MockCommit(
            "Fix critical security vulnerability in auth module", "Alice", 25, 10
        ),

...
class CostCalculator:
    ...
    def calculate_run_cost(
        self, duration_seconds: int, runner_type: str = "ubuntu", jobs_count: int = 1
    ) -> float:
    ...
    def calculate_optimization_savings(
        ...
        return {
            ...
            "percentage_improvement": ((old_duration - new_duration) / old_duration)
            * 100,
        }
    ...
    def generate_cost_report(
        self, runs_per_day: int = 10, avg_duration_seconds: int = 300
    ) -> str:
...
"""
        ...
        if tier_usage["overage_minutes"] > 0:
            recommendations.append(
                "🔴 **High Priority**: Reduce workflow runs to stay within free tier"
            )

        if avg_duration_seconds > 300:
            recommendations.append(
                "🟡 **Medium Priority**: Optimize workflow duration (currently above 5 min)"
            )

        if runs_per_day > 20:
            recommendations.append(
                "🟡 **Medium Priority**: Consider reducing test frequency or using smarter triggers"
            )
...
"""
    ...
    def calculate_roi(
        ...
        payback_months = (
            setup_cost / monthly_savings if monthly_savings > 0 else float("inf")
        )
        annual_roi = (
            ((monthly_savings * 12 - setup_cost) / setup_cost) * 100
            if setup_cost > 0
            else 0
        )

...
def apply_updates(repo_root: Path, results: list[da.AuditResult], use_claude: bool) -> str | None:
    ...
    if existing.stdout.strip():
        ...
        return None
    # Start the bump PR from latest main so it targets main cleanly regardless
    # of what branch the repo currently has checked out.
    _run(["git", "-C", str(repo_root), "fetch", "origin", "main"], check=False)
    _run(["git", "-C", str(repo_root), "checkout", "-b", branch, "origin/main"])
    for r in safe:
    ...
    # Push the bump branch, then open the PR against main. Pushing first avoids
    # `gh`'s "uncommitted change / use --head" abort when the working tree has
    # unrelated untracked files (e.g. .mcp.json). DRAFT ONLY -- never merges.
    push = _run(
        ["git", "-C", str(repo_root), "push", "-u", "origin", branch],
        check=False,
    )
    if push.returncode != 0:
        return f"PUSH FAILED: {push.stderr.strip() or push.stdout.strip()}"
    pr = _run(
        [
            "gh",
            "pr",

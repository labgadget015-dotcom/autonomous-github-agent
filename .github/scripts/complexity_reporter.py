...
"""
        ...
        if issues['high_complexity']:
            ...
            for item in sorted(issues['high_complexity'], key=lambda x: x['complexity'], reverse=True)[:10]:
        ...
        if not issues['high_complexity'] and not issues['low_maintainability']:
            report += "### ✅ All Clear!\n\n"
            report += "No complexity or maintainability issues detected. Great job! 🎉\n"
    ...
    def should_block_merge(self, issues: Dict) -> bool:
        ...
        critical_complexity = [i for i in issues['high_complexity'] if i['complexity'] > 20]
        critical_maintainability = [i for i in issues['low_maintainability'] if i['score'] < 40]

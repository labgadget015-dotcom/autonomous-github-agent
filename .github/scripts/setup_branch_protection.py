...
def setup_branch_protection(
    token: str, repository_name: str, branch_name: str = "main"
) -> bool:
    """
    ...
    try:
        ...
        print("Configuration:")
        print(
            f"  ✓ Require pull request reviews: {config['require_pull_request_reviews']['required_approving_review_count']} approval(s)"
        )
        print(
            f"  ✓ Dismiss stale reviews: {config['require_pull_request_reviews']['dismiss_stale_reviews']}"
        )
        ...
        print(
            f"  ✓ Require conversation resolution: {config['required_conversation_resolution']}"
        )

        # Edit branch protection using PyGithub
        branch.edit_protection(
            # Require pull request reviews
            required_approving_review_count=config["require_pull_request_reviews"][
                "required_approving_review_count"
            ],
            dismiss_stale_reviews=config["require_pull_request_reviews"][
                "dismiss_stale_reviews"
            ],
            require_code_owner_reviews=config["require_pull_request_reviews"][
        ...
        if e.status == 401:
            ...
            print(
                "Error: Token doesn't have sufficient permissions. Ensure the token has 'repo' scope and admin access."
            )
        elif e.status == 404:
            print(
                f"Error: Repository '{repository_name}' not found or token doesn't have access"
            )
        return False
...
def verify_branch_protection(
    ...
    try:
        ...
        if protection.required_pull_request_reviews:
            print(
                f"  • Required approving reviews: {protection.required_pull_request_reviews.required_approving_review_count}"
            )
            print(
                f"  • Dismiss stale reviews: {protection.required_pull_request_reviews.dismiss_stale_reviews}"
            )

        if protection.required_status_checks:
            print(
                f"  • Strict status checks: {protection.required_status_checks.strict}"
            )
        ...
        print(
            f"  • Enforce for admins: {protection.enforce_admins.enabled if protection.enforce_admins else False}"
        )
        print(
            f"  • Allow force pushes: {protection.allow_force_pushes.enabled if protection.allow_force_pushes else False}"
        )
        print(
            f"  • Allow deletions: {protection.allow_deletions.enabled if protection.allow_deletions else False}"
        )
...
def main():
    ...
    while True:
        response = (
            input("\nDo you want to apply/update branch protection rules? (yes/no): ")
            .strip()
            .lower()
        )

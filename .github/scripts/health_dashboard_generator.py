...
"""
    ...
    def _generate_complexity_section(self) -> str:
        ...
        if mi_data:
            scores = [v if isinstance(v, (int, float)) else v.get('mi', 100) 
...
"""
    ...
    def _generate_recommendations(self) -> str:
        ...
        if security_issues > 0:
            recommendations.append(f"🔒 **Fix {security_issues} security issues** detected by Bandit")
        ...
        if complexity_issues:
            recommendations.append(f"🔧 **Refactor {len(complexity_issues)} high-complexity functions**")

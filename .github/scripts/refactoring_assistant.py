...
class RefactoringAssistant:
    ...
    def _analyze_file(self, file_path: Path) -> List[RefactoringOpportunity]:
        ...
        try:
            ...
            opportunities.extend(self._find_complex_conditionals(file_path, tree, content))
        except SyntaxError:
            pass
        
        return opportunities
    
    def _find_long_methods(self, file_path: Path, tree: ast.AST, content: str) -> List[RefactoringOpportunity]:
    ...
    def _find_duplicate_code(self, file_path: Path, content: str) -> List[RefactoringOpportunity]:
    ...
    def _find_complex_conditionals(self, file_path: Path, tree: ast.AST, content: str) -> List[RefactoringOpportunity]:
    ...
    def _calculate_condition_complexity(self, node: ast.AST) -> int:
        ...
        if isinstance(node, (ast.And, ast.Or)):
            return 1 + sum(self._calculate_condition_complexity(val) for val in [node.left, node.right] if hasattr(node, 'left'))
    ...
    def generate_report(self, output_path: str = "REFACTORING_OPPORTUNITIES.md") -> None:
        ...
        for ref_type, opps in sorted(by_type.items()):
            ...
            report.append(f"{emoji} **{ref_type.replace('_', ' ').title()}**: {len(opps)} opportunities")
        ...
        for i, opp in enumerate(self.opportunities[:20], 1):  # Top 20
            ...
            report.append(f"- **File:** `{opp.file_path}` (Lines {opp.line_range[0]}-{opp.line_range[1]})")
            report.append(f"- **Impact:** {impact_emoji} {opp.impact.title()}")
            report.append(f"- **Confidence:** {opp.confidence * 100:.0f}%")
            report.append(f"- **Complexity:** {opp.complexity_before} → {opp.complexity_after}")
            report.append(f"- **Auto-applicable:** {'✅ Yes' if opp.auto_applicable else '❌ No'}")
    ...
    def estimate_time_savings(self) -> Dict[str, Any]:
        ...
        total_complexity_reduction = sum(
            o.complexity_before - o.complexity_after 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Automated Refactoring Assistant'
    )
    parser.add_argument('--repo-path', default='.', help='Path to repository')
    parser.add_argument('--output', default='REFACTORING_OPPORTUNITIES.md', 
                       help='Output file')
    ...
    try:
        ...
        if opportunities:
            ...
            print(f"   Time savings: {savings['estimated_time_savings_hours']:.1f} hours")

...
class ThresholdMonitor:
    ...
    def check_analysis_results(self, results_path: str = "analysis-results.json") -> List[Dict]:
    ...
    def create_github_issue(self, violation: Dict) -> Dict:
        ...
        labels = [
            'code-quality',
            self.config['escalation']['severity_labels'].get(severity, 'priority: medium')
        ]
        ...
        title = title_map.get(violation['type'], f"Code Quality Issue: {violation.get('message', '')}")
...
"""
    ...
    def save_violations(self, violations: List[Dict], path: str = "threshold-violations.json"):

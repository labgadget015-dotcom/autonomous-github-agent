...
class AsyncParallelAnalyzer:
    ...
    async def run_tool(self, tool_name: str, command: List[str]) -> AnalysisResult:
        ...
        try:
            ...
            print(f"{'✅' if success else '⚠️ '} {tool_name} completed in {duration:.2f}s ({issues} issues)")
    ...
    async def run_all_parallel(self) -> List[AnalysisResult]:
        ...
        tools = {
            'pylint': [
                'pylint', self.target_path,
                '--output-format=json',
                f'--output={self.reports_dir}/pylint-report.json',
                '--exit-zero'
            ],
            'flake8': [
                'flake8', self.target_path,
                '--format=json',
                f'--output-file={self.reports_dir}/flake8-report.json',
                '--exit-zero'
            ],
            'mypy': [
                'mypy', self.target_path,
                '--json-report', str(self.reports_dir / 'mypy'),
                '--ignore-missing-imports',
                '--no-error-summary'
            ],
            'bandit': [
                'bandit', '-r', self.target_path,
                '-f', 'json',
                '-o', str(self.reports_dir / 'bandit-report.json'),
                '--exit-zero'
            ],
            'radon_cc': [
                'radon', 'cc', self.target_path,
                '-a', '-j',
                '-O', str(self.reports_dir / 'radon-cc.json')
            ],
            'radon_mi': [
                'radon', 'mi', self.target_path,
                '-j',
                '-O', str(self.reports_dir / 'radon-mi.json')
            ]
        }
    ...
    def print_summary(self, summary: Dict[str, Any]) -> None:
        ...
        for result in summary['results']:
            ...
            print(f"{status} {result['tool']:15} {result['duration']:6.2f}s  {result['issues']:4d} issues")
            if result['error']:

...
class SelfHealingSystem:
    ...
    def analyze_failure_patterns(self) -> Dict[str, any]:
        ...
        recent = [f for f in self.failures if datetime.now() - f.timestamp < timedelta(hours=24)]
        ...
        return {
            ...
            'top_failing_component': max(by_component.items(), key=lambda x: len(x[1]))[0] if by_component else None
        }
    
    def get_health_score(self) -> float:
        """Calculate system health score (0-100)"""
        recent = [f for f in self.failures if datetime.now() - f.timestamp < timedelta(hours=1)]
...
async def demo_self_healing():
    ...
    system.register_circuit_breaker("github_api", CircuitBreaker("github_api", failure_threshold=3))
    system.register_circuit_breaker("cache_service", CircuitBreaker("cache_service", failure_threshold=5))

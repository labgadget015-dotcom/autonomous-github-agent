...
class MockCommit:
    ...
    def __init__(
        ...
        self.commit = type(
            "CommitData",
            (),
            {"message": message, "author": type("Author", (), {"name": author_name})},
        )
        self.stats = type(
            "Stats",
            (),
...
class TestAnomalyDetector:
    ...
    def test_detect_normal_change(self):
        ...
        # Establish baseline
        baseline_commits = [MockCommit("Normal", 10, 5) for _ in range(10)]
    ...
    def test_detect_anomalous_change(self):
        ...
        large_files = [
            type("File", (), {"filename": f"src/file{i}.py"}) for i in range(50)
        ]

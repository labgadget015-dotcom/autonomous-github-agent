...
class TestOrchestratorAgent:
    ...
    async def test_delegate_task_with_approval(self, orchestrator_config, mock_agent):
        ...
        with patch.object(orchestrator, '_determine_agent', return_value='test_agent'):
            ...
            with patch.object(agent.policy, 'check_action', new_callable=AsyncMock) as mock_policy:
                mock_policy.return_value = {
                    'requires_approval': True,
                    'reason': 'Test approval required'
                }
                
                with patch.object(orchestrator, '_create_approval_issue', new_callable=AsyncMock) as mock_create:
    ...
    async def test_route_task(self, orchestrator_config):
        ...
        with patch.object(orchestrator, '_determine_agent', return_value='test_agent'):
            with patch.object(orchestrator.message_queue, 'enqueue', new_callable=AsyncMock) as mock_enqueue:
    ...
    async def test_execute_parallel(self, orchestrator_config, mock_agent):
        ...
        with patch.object(orchestrator, '_determine_agent', return_value='test_agent'):
            ...
            tasks = [
                {'type': 'task1', 'data': {'action': 'test_action', 'params': {'id': 1}}},
                {'type': 'task2', 'data': {'action': 'test_action', 'params': {'id': 2}}},
                {'type': 'task3', 'data': {'action': 'test_action', 'params': {'id': 3}}}
            ]
    ...
    async def test_execute_sequential_stop_on_error(self, orchestrator_config):
        ...
        with patch.object(orchestrator, '_determine_agent', return_value='failing_agent'):

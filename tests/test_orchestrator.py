...
def orchestrator(mock_dependencies):
    ...
    with patch('autonomous_agent.core.orchestrator.get_config', return_value=mock_dependencies['config']):
        with patch('autonomous_agent.core.orchestrator.GitHubClient', return_value=mock_dependencies['github_client']):
            with patch('autonomous_agent.core.orchestrator.LLMClient', return_value=mock_dependencies['llm_client']):
                with patch('autonomous_agent.core.orchestrator.AuditLogger', return_value=mock_dependencies['audit_logger']):
                    orch = Orchestrator()
    return orch
    ...
    async def test_monitor_repository_basic(self, orchestrator):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                orchestrator.monitor_repository("owner/repo"),
                timeout=0.1
            )

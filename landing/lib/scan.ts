import type { CheckResult, ScanReport } from "./scan-types";

const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? "https://gadgetlab.uk";
...
function ghHeaders(): Record<string, string> {
  ...
  if (process.env.GH_PAT) h["Authorization"] = `Bearer ${process.env.GH_PAT}`;
  return h;
}

export async function runScan(owner: string, repo: string): Promise<ScanReport> {
  ...
  if (repoRes.status === 404) {
    const err = new Error("Repository not found or is private");
  ...
  if (!latestRun) {
    ...
    ciCheck = { label: "CI / CD", status: "critical", detail: `${latestRun.name} ${latestRun.conclusion ?? "failed"}`, points: 0, maxPoints: 30 };
  }
  ...
  const stalePrs = Array.isArray(prsData)
    ? prsData.filter((pr: { updated_at: string }) => {
        return (now - new Date(pr.updated_at).getTime()) / 86400000 > 30;
      }).length
    : 0;
  ...
  const daysSince = pushedAt ? (now - pushedAt.getTime()) / 86400000 : 999;
  ...
  const pageUrl = `${APP_URL}/scan/${owner}/${repo}`;
  const badgeUrl = `https://img.shields.io/endpoint?url=${APP_URL}/api/badge/${owner}/${repo}`;
  const embedCode = `[![GadgetLab Score](${badgeUrl})](${pageUrl})`;

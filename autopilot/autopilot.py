...
class GitHubAutopilot:
    ...
    def fetch_repo_data(self, owner: str, name: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            since_time = datetime.now() - timedelta(
                hours=self.config["analysis"]["lookback"]["commits"]
            )
    ...
    def generate_summary(self, priorities: list[dict]) -> str:
        ...
        for i, item in enumerate(priorities, 1):
            ...
            summary.append(
                f"{i}. **[{item['repo']}]** {item_type} #{item['number']}: {item['title']}"  # noqa: E501
            )
        ...
        # Group by priority
        critical_repos = [
            (k, v) for k, v in self.repo_data.items() if v["priority"] == "critical"
        ]
        medium_repos = [
            (k, v) for k, v in self.repo_data.items() if v["priority"] == "medium"
        ]
        low_repos = [
            (k, v) for k, v in self.repo_data.items() if v["priority"] == "low"
        ]

        for group_name, repos in [
            ("Critical Repos", critical_repos),
            ("Medium Priority", medium_repos),
            ("Low Priority (Paused)", low_repos),
        ]:
            if not repos:
                continue
            ...
            for repo_key, data in repos:
                ...
                summary.append(
                    f"#### {repo_key} ({issue_count} issues, {pr_count} PRs)\n"
                )

                if commit_count > 0:
                    summary.append(
                        f"- **Recent Activity:** {commit_count} commits in last 24h"
                    )
                ...
                if issue_count > 0:
                    urgent_issues = [
                        ...
                        if any("urgent" in lbl.name.lower() for lbl in i.labels)
                    ]
                    if urgent_issues:
                        summary.append(
                            f"- **Urgent Issues:** {len(urgent_issues)} urgent"
                        )
        ...
        summary.append("---\n")
        summary.append(
            f"*Generated by GitHub Autopilot v0 - {now.strftime('%Y-%m-%d %H:%M:%S GMT')}*"  # noqa: E501
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GitHub Autopilot - Daily Repository Summary"
    )

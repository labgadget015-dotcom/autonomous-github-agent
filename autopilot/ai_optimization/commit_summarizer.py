...
class CommitSummarizer:
    ...
    def generate_summary(self, commits: list[Any]) -> dict[str, Any]:
        ...
        # Generate insights
        insights = self._generate_insights(commits, themes, actions, components)
    ...
    def _generate_summary_text(
        ...
        parts.append(
            f"affecting {files_changed} file{'s' if files_changed != 1 else ''}"
        )
    ...
    def _generate_insights(
        ...
        for commit in commits:
            ...
            if "breaking" in message.lower() or "BREAKING" in message:
                insights.append(
                    "Breaking changes detected - version bump may be needed"
                )
                break
    ...
    class MockCommit:
        def __init__(self, message, author, additions, deletions):
            self.commit = type(
                "CommitData",
                (),
                {"message": message, "author": type("Author", (), {"name": author})},
            )

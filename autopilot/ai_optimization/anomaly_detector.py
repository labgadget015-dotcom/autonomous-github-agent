...
class AnomalyDetector:
    ...
    def _calculate_complexity(
        self, additions: int, deletions: int, files: int
    ) -> float:
    ...
    def detect_significant_changes(self, commit: Any) -> dict[str, Any]:
        ...
        if self.trained and self.baseline is not None:
            try:
                ...
                is_anomaly, anomaly_score, confidence = self._rule_based_detection(
                    features
                )
        else:
    ...
    def _rule_based_detection(self, features: np.ndarray) -> tuple[bool, float, float]:
        ...
        if self.baseline is None:
            ...
            is_anomaly = files_changed > 50 or total > 1000 or complexity > 5.0
    ...
    def _explain_anomaly(self, features: np.ndarray, baseline: dict | None) -> str:
        ...
        if test_ratio > 0.5:
            explanations.append(
                f"Primarily test changes ({test_ratio*100:.0f}% test files)"
            )

        if doc_ratio > 0.5:
            explanations.append(
                f"Primarily documentation ({doc_ratio*100:.0f}% doc files)"
            )
    ...
    def analyze_commit_batch(self, commits: list[Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "significance_rate": (
                significant_count / total_commits if total_commits > 0 else 0
            ),
...
if __name__ == "__main__":
    ...
    class MockCommit:
        def __init__(self, files, additions, deletions):
            ...
            self.sha = "abc123"
            self.commit = type(
                "CommitData", (), {"message": f"Example commit with {len(files)} files"}
            )
    ...
    test_commits = [
        ...
        (
            "Large refactor",
            MockCommit([f"src/file{i}.py" for i in range(30)], 500, 300),
        ),
        (
            "Massive feature",
            MockCommit([f"feature/file{i}.py" for i in range(50)], 2000, 100),
        ),

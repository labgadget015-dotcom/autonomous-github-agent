...
class DependencyManager:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Found {len(results['vulnerabilities'])} vulnerabilities, {len(results['outdated'])} outdated packages"
        )
    ...
    def _check_vulnerability(
        ...
        if package in known_vulns:
            ...
            if version and self._is_vulnerable_version(
                version, vuln["version"], vuln["operator"]
            ):
                return {
                    "package": package,
                    "version": version,
    ...
    def _is_vulnerable_version(
        self, current: str, vuln_version: str, operator: str
    ) -> bool:

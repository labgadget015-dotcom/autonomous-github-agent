...
class CICDOptimizer:
    ...
    def analyze(self) -> dict[str, Any]:
        ...
        logger.info(
            f"CI/CD analysis complete. Found {len(results['optimizations'])} optimization opportunities"
        )
    ...
    def _check_missing_workflows(self, results: dict):
        ...
        has_test_workflow = any(
            (self.workflows_dir / name).exists() for name in test_workflows
        )
        ...
        has_lint_workflow = any(
            (self.workflows_dir / name).exists() for name in lint_workflows
        )
        ...
        has_deploy_workflow = any(
            (self.workflows_dir / name).exists() for name in deploy_workflows
        )

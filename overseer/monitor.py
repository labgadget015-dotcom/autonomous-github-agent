...
class RepositoryMonitor:
    ...
    def _monitor_code_quality(self) -> Dict[str, Any]:
        ...
        linting_configs = ['.pylintrc', '.flake8', 'setup.cfg', 'pyproject.toml', '.eslintrc.js', '.eslintrc.json']
        quality['has_linting'] = any((self.repo_path / config).exists() for config in linting_configs)
    ...
    def _monitor_security(self) -> Dict[str, Any]:
        ...
        security['has_security_policy'] = any((self.repo_path / f).exists() for f in security_files)
        ...
        if workflows_dir.exists():
            security_workflows = [f for f in workflows_dir.glob('*.yml') 
    ...
    def _monitor_collaboration(self) -> Dict[str, Any]:
        ...
        collaboration['has_contributing_guide'] = any((self.repo_path / f).exists() for f in contrib_files)
        
        # Check for CODE_OF_CONDUCT.md
        coc_files = ['CODE_OF_CONDUCT.md', '.github/CODE_OF_CONDUCT.md']
        collaboration['has_code_of_conduct'] = any((self.repo_path / f).exists() for f in coc_files)
        ...
        collaboration['has_pr_template'] = any((self.repo_path / t).exists() for t in pr_templates)

...
class OrchestratorAgent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Orchestrator initialized with routing strategy: {self.routing_strategy}")
    ...
    async def _execute_parallel(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Executing {len(tasks)} tasks in parallel (batch size: {batch_size})")
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            coroutines = [
                self._delegate_task({'task_type': t.get('type'), 'task_data': t.get('data')})
                for t in batch
            ]
    ...
    async def _execute_sequential(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for task in tasks:
            ...
            if result.get('status') == 'error' and params.get('stop_on_error', False):
                logger.warning(f"Stopping execution due to error in task: {task.get('type')}")
                break

...
def demo_code_analysis(overseer: RepositoryOverseer):
    ...
    print("\n🔍 Issues Found:")
    print(f"   • Refactoring Opportunities: {len(results['refactoring_opportunities'])}")
...
def demo_cicd(overseer: RepositoryOverseer):
    ...
    print(f"   • Deployment Recommendations: {len(results['deployment_recommendations'])}")
...
def demo_issues(overseer: RepositoryOverseer):
    ...
    sample_issues = [
        ("Critical bug in authentication", "The login feature is broken and causing security issues"),

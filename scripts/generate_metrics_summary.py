...
class MetricsSummaryAgent:
    ...
    def __init__(self, output_format: str = "markdown"):
        ...
        self.generate_daily = (
            os.getenv("GENERATE_DAILY_REPORTS", "true").lower() == "true"
        )
        self.generate_weekly = (
            os.getenv("GENERATE_WEEKLY_SUMMARIES", "true").lower() == "true"
        )
        ...
        if metrics:
            ...
            lines.extend(
                [
                    ...
                    "",
                    (
                        f"**Last Run:** {datetime.fromtimestamp(recent['_timestamp']).strftime('%Y-%m-%d %H:%M:%S')}"
                        if "_timestamp" in recent
                        else "Unknown"
                    ),
        ...
        for key in ["coverage", "complexity", "quality_score", "workflow_duration"]:
            ...
            for m in metrics:
                if key in m and "_timestamp" in m:
                    values.append(m[key])
                    timestamps.append(
                        datetime.fromtimestamp(m["_timestamp"]).isoformat()
                    )
...
def main():
    ...
    parser.add_argument(
        "--metrics-dir", required=True, help="Directory containing metrics JSON files"
    )
    parser.add_argument(
        "--output", default="metrics_summary.md", help="Output file path"
    )
    parser.add_argument(
        "--format",

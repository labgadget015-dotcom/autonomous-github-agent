...
class ComprehensiveDocGenerator:
    ...
    def _analyze_project(self) -> Dict:
        ...
        info = {
            ...
            'has_python': (self.repo_path / 'requirements.txt').exists() or 
            ...
            'has_tests': (self.repo_path / 'tests').exists() or 
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.ClassDef):
                            ...
                            if methods:
                                ...
                                for method in methods:
                                    ...
                                    content.append(f"##### `{method.name}({', '.join(args)})`\n\n")
                        ...
                        # Document module-level functions
                            ...
                            if not node.name.startswith('_'):
                                ...
                                content.append(f"### Function: `{node.name}({', '.join(args)})`\n\n")
    ...
    def generate_architecture_doc(self) -> str:
        ...
        if self.project_info['has_python'] and (self.repo_path / 'requirements.txt').exists():
            content.append("### Python Dependencies\n\n")
            content.append("See `requirements.txt` for the full list.\n\n")
        
        if self.project_info['has_javascript'] and (self.repo_path / 'package.json').exists():

...
class QualityEscalator:
    ...
    def __init__(self, repo_name: str, token: str):
        ...
        self.issue_assignees = os.getenv('ISSUE_ASSIGNEES', '').split(',') if os.getenv('ISSUE_ASSIGNEES') else []
    ...
    def check_coverage_drop(self, current_coverage: float, previous_coverage: float) -> Optional[Dict]:
        """Check if test coverage has dropped significantly."""
        if previous_coverage == 0:
            return None
        
        drop_percent = ((previous_coverage - current_coverage) / previous_coverage) * 100
    ...
    def check_security_violations(self, security_data: Dict) -> Optional[Dict]:
        ...
        if critical > self.thresholds['max_critical_vulns'] or high > self.thresholds['max_high_vulns']:
            return {
                'type': 'security',
    ...
    def create_issue(self, violation: Dict, pr_number: Optional[int] = None) -> Optional[str]:
        """Create a GitHub issue for the violation."""
        if not self.auto_create:
            print(f"Auto-creation disabled. Would create issue for: {violation['summary']}")
            return None
        ...
        if violation['type'] == 'complexity':
            ...
            for v in violation['violations'][:10]:  # Limit to 10 files
                body_parts.append(f"| {v['file']} | {v['complexity']} | {v['threshold']} |")

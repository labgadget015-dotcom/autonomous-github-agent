...
class LocalTester:
    ...
    def run_command(self, cmd: List[str], description: str) -> Tuple[bool, str]:
        ...
        try:
            result = subprocess.run(
                cmd, capture_output=not self.verbose, text=True, timeout=300
            )
    ...
    def check_isort(self) -> bool:
        ...
        success, _ = self.run_command(
            ["isort", "--check-only", "."], "Checking import sorting (isort)"
        )
    ...
    def check_pylint(self) -> bool:
        ...
        success, _ = self.run_command(
            ["pylint", ".github/scripts", "--fail-under=8.0"], "Running Pylint analysis"
        )
